...
fn open_preview(app: tauri::AppHandle, html: String) -> Result<(), String> {
    ...
    tauri::WebviewWindowBuilder::new(&app, "preview", tauri::WebviewUrl::External(url.parse().map_err(|e| format!("{e}"))?)).title("Apercu du projet").inner_size(1000.0, 700.0).build().map_err(|e| e.to_string())?;
    Ok(())
}
fn main() {
